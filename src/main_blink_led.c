...
int main(void) {
    // Enable clock on GPIOA (peripherals are clock-gated by default to save power)
    RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN;

    // Configure PA0 as input
    // 3 in binary is 0b11, so ~3 is 0b00
    // From reference manual, 00 is input / reset state
    GPIOA->MODER &= ~(3U << 0);
